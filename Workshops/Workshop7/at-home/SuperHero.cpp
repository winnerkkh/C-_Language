...
namespace sict{
	...
	SuperHero::SuperHero() : Hero() {
		...
		// initialize all the extra instance members of SuperHero
		// rest of the members got received from Hero class
	...
	SuperHero::SuperHero( const char* name, int maximumHealth, int attack, int superPowerAttack, int superPowerDefend)  : Hero(name, maximumHealth, attack) { 
		...
		//derived class = > I do not need to assign one more because I get all the data members from Hero class
	...
	int SuperHero::getAttack() const { 
		...
		//shadow concept 
		//using getAttack function from Hero class plus using SuperHero class attack bonus
